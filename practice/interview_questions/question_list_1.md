...
### Find if any element matches a condition (e.g., is any string empty?). 
...
### Custom Collector to count vowel and consonant words. 
### Implement pagination (skip & limit) using streams. 
### Find all anagrams in a list of strings. 
### Parallel stream to process a large data set and measure performance. 
### Create a nested map: Department → Role → List<Employee>. 
### Find the top N most frequent words from a text. 
### Use reducing() collector to concatenate strings with custom separator. 
